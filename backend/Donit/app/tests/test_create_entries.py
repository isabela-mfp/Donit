...
class TestCreateTasksOnDatabase(TestCase):
    def setUp(self):
        self.user1 = User(username='dummy')
        self.user1.save()
        self.test_list = ListManagement(name='Lista 1', userid=self.user1, description='Esta eh a primeira lista de teste', type='A')
        self.test_list.save()
        self.task1 = TaskManagement(name='Tarefa 1', listid=self.test_list, description='Esta eh a primeira tarefa de teste', conclusion=datetime.date(2022, 9, 9), priority=1, status='T')
        self.task2 = TaskManagement(name='Tarefa 2', listid=self.test_list, description='Exemplo de tarefa do supermercado', conclusion=datetime.date(2022, 9, 9), priority=1, status='T')
        self.task1.save()
        self.task2.save()   
    ...
    def test_get_and_compare_tasks_on_database(self):
        ...
        self.assertEqual(first_saved_item, self.task1)
        self.assertEqual(second_saved_item.name, 'Tarefa 2')
        self.assertEqual(second_saved_item, self.task2)
